...
export default function Header() {
  ...
  const logoSrc = theme === 'dark' ? '/images/logoacik.png' : '/images/logokoyu.png';
  ...
  const mobileLinkClass = (href: string) => {
    ...
    const idle = 'text-amber-100 hover:bg-[#1d356f]';
  ...
  return (
    <header className="sticky top-0 z-50 px-3 pt-3 sm:px-5 lg:px-8">
      ...
      <div className="relative mx-auto max-w-screen-2xl">
        <div className="flex h-[76px] items-center gap-2 rounded-2xl border border-amber-200/25 bg-[#142862]/92 px-3 shadow-[0_18px_55px_rgba(8,18,50,0.5)] ring-1 ring-amber-100/10 backdrop-blur-2xl sm:gap-3 sm:px-4">
          ...
          <nav className="hidden flex-1 justify-center lg:flex" aria-label="Ana menü">
            <div className="flex items-center gap-1.5 rounded-full border border-amber-200/25 bg-[#142862]/80 p-1.5">
              {NAV_ITEMS.map((item) => (
          ...
          <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
            ...
            <a href={whatsAppHref} target="_blank" rel="noreferrer" className="hidden xl:inline-flex items-center justify-center rounded-full border border-amber-200/35 bg-[#1b3370] px-4 py-2 text-xs font-semibold tracking-[0.01em] text-amber-100 transition hover:border-amber-300/70 hover:bg-[#223f86]">
              WhatsApp
            </a>

            <button type="button" onClick={toggleTheme} className="hidden sm:inline-flex items-center justify-center rounded-full border border-amber-200/35 bg-[#1b3370] p-2 text-amber-100 transition hover:bg-[#223f86] hover:text-amber-50" aria-label="Tema değiştir">
              {theme === 'dark' ? (
            ...
            {isAuthed && (
              <button type="button" onClick={openNotifications} className="relative inline-flex items-center justify-center rounded-full p-2 text-amber-100/80 transition hover:bg-[#1d356f] hover:text-amber-50" aria-label="Bildirimleri aç">
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                {notificationsBadge}
              </button>
            )}

            <button type="button" onClick={toggleCart} className="relative inline-flex items-center justify-center rounded-full p-2 text-amber-100/80 transition hover:bg-[#1d356f] hover:text-amber-50" aria-label="Sepeti aç">
            ...
            {isAuthed ? (
              <button type="button" onClick={() => setProfileOpen(true)} className="hidden sm:inline-flex items-center justify-center rounded-full border border-amber-200/35 bg-[#1b3370] p-1.5 text-amber-100 transition hover:bg-[#223f86]" aria-label="Profil">
                {avatarUrl ? (
              ...
              <div className="hidden items-center gap-2 sm:flex">
                <Link href={loginHref} className="inline-flex items-center justify-center rounded-full border border-amber-200/35 bg-[#1b3370] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-amber-100 transition hover:bg-[#223f86]">Giriş</Link>
            ...
            <button type="button" className="z-10 shrink-0 inline-flex items-center justify-center rounded-full border border-amber-200/35 bg-[#1b3370] p-2 text-amber-100 transition hover:bg-[#223f86] lg:hidden" onClick={() => setMobileMenuOpen((prev) => !prev)} aria-expanded={mobileMenuOpen} aria-label={mobileMenuOpen ? 'Menüyü kapat' : 'Menüyü aç'}>
      ...
      <div className={`fixed inset-0 z-[150] lg:hidden ${mobileMenuOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        <aside
          ...
          aria-modal="true"
          className={`absolute inset-0 h-dvh overflow-y-auto bg-[#142862] px-6 pb-10 pt-6 transition-opacity duration-200 ${mobileMenuOpen ? 'opacity-100' : 'opacity-0'}`}
        >
          <div className="mb-8 flex items-center justify-between border-b border-amber-200/15 pb-5">
            ...
            <button
              ...
              className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-amber-200/30 bg-[#1b3370] text-amber-100"
          ...
          {!isAuthed ? (
            <div className="mt-8 grid gap-3">
              <Link
                href={loginHref}
                onClick={() => setMobileMenuOpen(false)}
                className="inline-flex items-center justify-center rounded-2xl border border-amber-200/35 bg-[#1b3370] px-4 py-3 text-sm font-semibold text-amber-100 transition hover:bg-[#223f86]"
              >
                Giriş Yap
              </Link>
              <Link
